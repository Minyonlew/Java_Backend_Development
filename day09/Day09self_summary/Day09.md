...
- **接口与抽象类的区别**
  - **抽象类：** 
    - 含有抽象方法的一定是抽象类，但抽象类可以有不是抽象方法的方法。
    - 抽象方法不需要具体实现方法，所以不能用抽象类创建对象。
    - 如果一个类继承于一个抽象类，那么这个类一定要实现父类的抽象方法。
    - 抽象类和抽象方法 默认是 public abstract 修饰。
  - **接口：**
    -  接口比抽象类还要“抽象”。
    - 接口中的变量 只能是 public static final变量。
    -  方法 只能是 public abstract方法，也就是不能有具体的实现（抽象方法）。
    - 当类Implements了一个接口时，要实现接口的所有方法。
  - **接口与抽象类的区别：**
    - 抽象类可以提供成员方法的实现细节（可以有非抽象方法），而接口只能含有抽象方法。
    -  抽象类的成员变量可以是各种类型的，而接口中的只能是public static final。
...
- 成员内部类
  ...
  - **匿名内部类**
    ...
    - **程序需要在匿名内部类中使用局部变量，那么这个局部变量必须使用`final`修饰符**！！！！
